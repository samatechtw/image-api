...
describe('workerService', () => {
  ...
  it('handlePath() single', async () => {
    const sourcePath = path.resolve(testAsset, 'wtm_256x256.jpeg')
  ...
  it('handlePath() multiple', async () => {
    const sourcePath = path.resolve(testAsset, 'wtm_256x256.jpeg')
    const config1: IImageJobConfig = {
      inputFormat: EnumFileFormat.jpeg,
      width: 48,
      height: 48,
    }
    const config2: IImageJobConfig = {
      inputFormat: EnumFileFormat.jpeg,
      outputFormat: EnumFileFormat.png,
    }
    const config3: IImageJobConfig = {
      inputFormat: EnumFileFormat.jpeg,
      optimizeAlgo: EnumOptimizationAlgorithm.mozjpeg,
      quality: 90,
    }
    const config4: IImageJobConfig = {
      inputFormat: EnumFileFormat.jpeg,
      outputFormat: EnumFileFormat.png,
      width: 48,
      height: 48,
